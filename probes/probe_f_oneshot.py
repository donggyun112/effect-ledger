...
class Ledger(AgentMiddleware):
    ...
    def wrap_tool_call(self, request, handler):  # noqa: ANN001, ANN201
        ...
        if state == "started":
            ...
            if call_id in self.spent:
                TRACE.append("ledger:verdict_spent")
                return ToolMessage(
                    content="INDETERMINATE: Verdict consumed; a new retry requires a new verdict.",
...
def main() -> None:
    ...
    config = {"configurable": {"thread_id": "probe-f"}}
    ...
    print()
    print("Charge count:", len(CHARGES))
    print("Human answers:", ASKS)
    print("Ledger:", ledger.records, "| Consumed verdicts:", ledger.spent)
    print("trace:", TRACE)
