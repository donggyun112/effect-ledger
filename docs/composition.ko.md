...
기존 LangChain 툴에는 [ExecutionBoundary](langchain-boundary.ko.md)를 진입점으로 사용한다.
...
`OperationStore`는 상속이 필수인 기본 클래스가 아닌 Protocol이다.

| 메서드 | 필요한 원자성/내구성 |
|---|---|
| `get(scope, id)` | 커밋된 Operation 또는 None 반환 |
| `unresolved(scope, limit)` | 판정 대기 중인 Operation 목록 반환; 읽기 전용이며 아무 권한도 주지 않음 |
