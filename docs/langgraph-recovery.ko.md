...
*[English](langgraph-recovery.md)*

> **이 문서는 보조 경로다.** 대부분의 경우 [ExecutionBoundary 가이드](langchain-boundary.ko.md)를
...
- 제공자 조회는 호스트의 RecoveryPolicy로 연결할 수 있다. 기본값은 수동 판정이며,
  시간만으로 재시도 안전을 추론하지 않는다. [조합 API](composition.ko.md)를 참조한다.
